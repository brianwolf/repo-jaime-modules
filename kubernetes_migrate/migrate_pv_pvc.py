...
pvcs = [
    ob
    for ob
    in tools.sh(f'kubectl get pvc -n {namespace} -o custom-columns=NAME:.metadata.name', echo=False).split('\n')[1:]
]
...
for pvc in pvcs:
    ...
    tools.sh(
        f'kubectl get pvc {pvc} -n {namespace} -o yaml > yamls/pvcs/{pvc}.yaml')
    ...
    tools.sh(f'kubectl get pv {pv} -o yaml > yamls/pvs/{pv}.yaml')
...
for pv in pvs_to_migrate:
    try:
        ...
        tools.sh(f'kubectl apply -f yamls/pvs/{pv}.yaml')
...
for pvc in pvcs_to_migrate:
    try:
        ...
        tools.sh(f'kubectl apply -n {namespace} -f yamls/pvcs/{pvc}.yaml')
