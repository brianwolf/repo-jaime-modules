...
# file BK
bk_file_path = f'/data/migrate_services_{cluster_from}_{namespace}_to_{cluster_to}_{namespace}.txt'
...
objects = [
    ob
    for ob
    in tools.sh(f'kubectl get services -n {namespace} -o custom-columns=NAME:.metadata.name', echo=False).split('\n')[1:]
]
...
for ob in objects:
    ...
    tools.sh(
        f'kubectl get services {ob} -n {namespace} -o yaml > yamls/{ob}.yaml')
