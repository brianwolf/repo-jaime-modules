...
objects = [
    ob
    for ob
    in tools.sh(f'kubectl get {object_from} -n {namespace_from} -o custom-columns=NAME:.metadata.name', echo=False).split('\n')[1:]
]
...
for ob in objects:
    ...
    tools.sh(
        f'kubectl get {object_from} {ob} -n {namespace_from} -o yaml > yamls/{ob}.yaml')
...
# migrar yamls
tools.sh(f'kubectl new-project {namespace_to}')
...
for ob in objects_to_migrate:
    try:
        ...
        tools.sh(f'kubectl {method_to} -n {namespace_to} -f yamls/{ob}.yaml')
