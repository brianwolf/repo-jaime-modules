...
secrets = [
    ob
    for ob
    in tools.sh(f'kubectl get secret -n {namespace} -o custom-columns=NAME:.metadata.name', echo=False).split('\n')[1:]
]
...
images = [
    ob
    for ob
    in tools.sh(f'kubectl get is -n {namespace} -o custom-columns=NAME:.metadata.name', echo=False).split('\n')[1:]
]
...
tools.sh(
    f'kubectl get secret {secret} -n {namespace} -o yaml > yamls/secrets/{secret}.yaml')
...
tools.sh(f'kubectl apply -n {namespace} -f yamls/secrets/{secret}.yaml')
