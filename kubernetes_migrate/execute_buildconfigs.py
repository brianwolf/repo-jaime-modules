...
# login 
login_success = tools.login_kubernetes(cluster)
...
bc_list = [
    bc
    for bc
    in tools.sh(f'kubectl get bc -n {namespace} -o custom-columns=NAME:.metadata.name').split('\n')[1:]
]
...
for bc in bc_list_to_execute:
    ...
    tools.sh(f'kubectl start-build {bc} -n {namespace}')
